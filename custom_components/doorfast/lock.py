...
async def async_setup_entry(hass, entry, async_add_entities): async_add_entities([DoorfastLock(hass.data[DOMAIN][entry.entry_id])])
class DoorfastLock(LockEntity):
    _attr_translation_key = "unlock"
    def __init__(self, client): self.client=client; self._locked=True
    @property
    def unique_id(self): return f"{DOMAIN}_unlock"
    @property
